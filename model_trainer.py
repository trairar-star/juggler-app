...
def train_models(train_df, predict_df, features, shop_hyperparams):
    ...
    for mode in ['change', 'keep']:
        ...
        # --- 曜日別モデルの学習 ---
        # 据え置き予測では深い型推測(曜日等)を行わない
        if mode == 'change' and 'target_weekday' in train_df_common.columns:
            ...
            for wd in sorted(train_df_common['target_weekday'].unique()):
                ...
                if len(wd_train_cls) >= 100 and y_wd_check.sum() >= 3:
                    X_wd = wd_train[current_features]
                    ...
                    try:
                        ...
                        wd_reg.fit(X_wd, y_wd_reg, sample_weight=sw_wd, categorical_feature=current_cat_features)
                        ...
                        wd_model.fit(X_wd_cls, y_wd_cls, sample_weight=sw_wd_cls, categorical_feature=current_cat_features)
                        corrs_wd = get_correlations(wd_train.assign(predicted_diff=X_wd_stacked['predicted_diff']), stacked_features)
                        feature_importances_list.append(pd.DataFrame({
                            'shop_name': f"{weekdays_map.get(wd, f'曜日{wd}')}({mode_label})",
                            'category': '曜日',
                            'feature': stacked_features,
        ...
        # --- イベント有無別モデルの学習 ---
        # 据え置き予測ではイベント推測を行わない
        if mode == 'change' and 'イベント名' in train_df_common.columns:
            ...
            for ev_type in ['通常日', 'イベント日']:
                ...
                if len(ev_train_cls) >= 100 and y_ev_check.sum() >= 3:
                    X_ev = ev_train[current_features]
                    ...
                    try:
                        ...
                        ev_reg.fit(X_ev, y_ev_reg, sample_weight=sw_ev, categorical_feature=current_cat_features)
                        ...
                        ev_model.fit(X_ev_cls, y_ev_cls, sample_weight=sw_ev_cls, categorical_feature=current_cat_features)
                        corrs_ev = get_correlations(ev_train.assign(predicted_diff=X_ev_stacked['predicted_diff']), stacked_features)
                        feature_importances_list.append(pd.DataFrame({
                            'shop_name': f"{ev_type}({mode_label})",
                            'category': 'イベント',
                            'feature': stacked_features,
